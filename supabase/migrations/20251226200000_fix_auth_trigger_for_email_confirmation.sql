-- Migration: Fix auth trigger to properly handle email confirmation for vendor and delivery agent creation
-- This ensures that profiles, vendors, and delivery_agents records are created when email is confirmed
...
BEGIN
  ...
  -- This can happen either at user creation (if auto-confirm is enabled) or later on update
  IF NEW.email_confirmed_at IS NOT NULL THEN
    ...
    -- Only create these if the profile was actually inserted (not updated)
    IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = NEW.id AND created_at < NOW() - INTERVAL '1 second') THEN
