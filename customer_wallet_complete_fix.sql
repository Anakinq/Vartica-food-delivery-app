...
SELECT 'Transaction columns:' as info, column_name, data_type
FROM information_schema.columns
WHERE table_name = 'customer_wallet_transactions'
ORDER BY ordinal_position;

SELECT 'Functions created:' as info, proname
FROM pg_proc
WHERE pronamespace = 'public'::regnamespace
AND proname IN ('top_up_wallet', 'deduct_wallet_balance', 'get_customer_wallet', 'get_customer_transactions', 'refund_order_to_wallet');

SELECT 'Sample wallet data:' as info, * FROM customer_wallets LIMIT 5;
SELECT 'Sample transaction data:' as info, * FROM customer_wallet_transactions LIMIT 5;
